...
public class IconCache {
    ...
    private static class ContentIconProvider implements IconProvider {
        private Icon createIcon(String name) {
            return new ImageIcon(this.getClass().getResource("/res/mime/" + name + ".png"));
        }
        ...
        public Icon getIcon(File iconType) throws IOException {
            ...
            if ("".equals(ext) && !name.startsWith(".")) {
                ...
                if (mimeType != null) {
                    ...
                    for (String type : new String[]{"getLabel", "image", "video", "audio", "application"}) {
            ...
            return this.createIcon("file");
        }
    }

    private static IconProvider iconProvider = new SystemIconProvider();

    private static final Map<String, Icon> cache = Collections.synchronizedMap(new HashMap<String, Icon>());
